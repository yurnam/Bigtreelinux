#!/usr/bin/env bash
# build-minimal-fb-kernel.sh – Build and flash the framebuffer Linux kernel
#                               for the BigTreeTech PandaTouch
#
# Run this script from the repository root:
#   ./build-minimal-fb-kernel.sh
#   ./build-minimal-fb-kernel.sh -p /dev/ttyUSB0
#   ./build-minimal-fb-kernel.sh --no-flash
#   ./build-minimal-fb-kernel.sh --clean    # force full rebuild from scratch
#
# What this builds (kernel + framebuffer)
# ────────────────────────────────────────
...
info()    { echo -e "${BLUE}[build-minimal-fb-kernel]${NC} $*"; }
success() { echo -e "${GREEN}[build-minimal-fb-kernel]${NC} $*"; }
warn()    { echo -e "${YELLOW}[build-minimal-fb-kernel]${NC} $*"; }
die()     { echo -e "${RED}[build-minimal-fb-kernel] ERROR:${NC} $*" >&2; exit 1; }

# ── Usage ─────────────────────────────────────────────────────────────────
usage() {
cat <<EOF
${BOLD}build-minimal-fb-kernel.sh${NC} – Build and flash the framebuffer Linux kernel
                             for the BigTreeTech PandaTouch

${BOLD}Usage:${NC}
  ./build-minimal-fb-kernel.sh [OPTIONS]
...
${BOLD}Examples:${NC}
  ./build-minimal-fb-kernel.sh                   # incremental build + flash (auto-detect port)
  ./build-minimal-fb-kernel.sh -p /dev/ttyUSB0  # specify serial port
  ./build-minimal-fb-kernel.sh --no-flash        # build only, no flashing
  ./build-minimal-fb-kernel.sh --clean           # full rebuild from scratch
  ./build-minimal-fb-kernel.sh --skip-build -p /dev/ttyUSB0  # flash pre-built images

${BOLD}Build times:${NC}
  First run:       ~60 min (toolchain + kernel + esp-hosted)
  Subsequent runs: ~5–10 min (incremental; only changed parts rebuild)

${BOLD}What is built (kernel + framebuffer):${NC}
  • Linux kernel: framebuffer (/dev/fb0), fbcon, GT911 touch driver
  • Rootfs: BusyBox + fbset
  • Writable /etc (jffs2) with LCD init script
...
run_docker_build() {
    local docker_args=(
        run --rm -it
        --name pandatouch-linux-build-fb-kernel
        --user "$(id -u):$(id -g)"
        --group-add dialout
        -v "$LINUX_DIR:/app"
...
main() {
    echo ""
    echo -e "${BOLD}╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║  PandaTouch Linux – Kernel + Framebuffer Build   ║${NC}"
    echo -e "${BOLD}╚══════════════════════════════════════════════════╝${NC}"
    echo ""
...
print_done() {
    ...
    if [[ "$NO_FLASH" -eq 1 ]]; then
        ...
        echo "    ./build-minimal-fb-kernel.sh --skip-build -p /dev/ttyUSB0"
    else
        ...
        echo "  4. The LCD should display the Linux console (fbcon)"
        echo "     /dev/fb0 is available for framebuffer applications"
        echo ""
